...
Herb Vision 是一款面向中医药教学、临床参考与大众科普的中草药智能识别微信小程序。用户通过拍照或上传图片，由多模态大模型（智谱 GLM-4V）进行识别，并返回符合《中国药典》规范的药材结构化信息；同时配套药材知识库、相似药材对比、个性化学习与用户反馈等功能，形成"识别 — 学习 — 对比 — 反馈"的完整闭环。
...
### 核心功能
- 📷 **AI 拍照识药**：上传中草药图片，调用 GLM-4V 多模态模型识别，返回药典标准名称、来源、性味归经、功效主治、用法用量、禁忌、外观特征、真伪鉴别要点、易混淆药材等结构化结果；
...
| AI 模型 | 智谱 GLM-4V / GLM-4 | 多模态图像识别 + 文本模糊搜索 |
...
4. 后端将图片转 base64，构造 GLM-4V 多模态请求，附带《中国药典》标准的结构化提示词；
...
| `ZHIPU_API_KEY` | 智谱 AI 平台 API Key |
| `ZHIPU_MODEL` | 多模态模型名称（默认 glm-4v） |
...
1. **多模态大模型落地**：将 GLM-4V 视觉大模型应用于中医药垂直领域，输出符合药典规范的结构化结果，而非通用图像描述；
